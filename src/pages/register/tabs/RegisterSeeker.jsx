...
// APIs
import { SeekerCreateRequest } from "@/requests";
...
const RegisterSeeker = () => {
    ...
    const handleRegisterSeeker = () => {
        ...
        SeekerCreateRequest(dataToSubmit)
            .then((res) => res.json())
            .then((data) => {
                _isLoading(false);

                if (data?.id) {
                    const seekerInfo = { id: data?.id, role: data?.role, email: data?.email, name: data?.name }
                    localStorage.setItem("userInfo", JSON.stringify(seekerInfo));
    ...
    return <div className="mt-8">{SECTION_ONE()}</div>;
};
